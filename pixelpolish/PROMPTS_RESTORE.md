...
## Правка 03.09, вечер: фон

Строка `Background: render a plain smooth studio backdrop` была ОШИБКОЙ.
Модель честно выполнила указание и НАРИСОВАЛА студийный задник — облачный,
в пятнах, — которого на пластине не было: там ровный тёмный фон.
Мы велели выдумать фон вместо того, чтобы велеть сохранить настоящий.

Правильная формулировка теперь в блоке 1: сохранить исходный фон как есть,
убрать с него только повреждения, ничего не дорисовывать.

Общее правило, которое из этого следует: в промпте реставрации любая фраза
вида «render», «create», «add» применительно к содержанию кадра — опасна.
Реставрация только УБИРАЕТ повреждения и ВОССТАНАВЛИВАЕТ то, что было.
Проверять каждую строку промпта на этот признак.

---

## Мелкий регулярный узор: правило масштаба (03.09, после трёх неудач)

СИМПТОМ. Клетчатая ткань после реставрации «течёт»: прямые нити плавно
изгибаются волнами, узор идёт кусками с мутными провалами между ними,
появляются тёмные бесформенные наросты, местами узор просвечивает сквозь
себя как двойная экспозиция. Особенно на собранной в складки оборке.

ПРИЧИНА, и она НЕ в силе модели. Kontext считает весь кадр в 1504x1760.
Оборка юбки занимает от кадра малую часть, и на одну клеточку узора
приходится порядка ДЕСЯТИ пикселей. На такой мелкоте диффузионная модель
не удерживает регулярную сетку: она понимает «здесь клетка», но не может
уложить прямые нити в десять пикселей и начинает их гнуть.
Понижение guidance и переформулировки промпта этого НЕ ЛЕЧАТ — пробовали.

ЛЕЧЕНИЕ — МАСШТАБ, а не параметры:
1. Вырезать проблемный участок с запасом не меньше двух периодов узора
   со всех сторон — модели нужен образец, который она продолжает.
2. УВЕЛИЧИТЬ КРОП ДО ПОЛНОГО РАЗМЕРА МОДЕЛИ (длинная сторона 1504-1760)
   перед прогоном. Это ключевой шаг. Если подать кроп в его натуральном
   мелком размере, модель посчитает те же десять пикселей на клетку и
   ничего не изменится.
3. Прогнать через Kontext с инструкцией именно про жёсткую ткань в складках:
   узор непрерывный, нити прямые, на складке ломаются под углом, а не
   изгибаются плавно.
4. Уменьшить обратно и вставить с растушёвкой около 24 px, сведя тон по краям.

ПРОВЕРКА. Спектр внутри починенного участка должен показать тот же основной
период узора, что и снаружи, а локальная энергия градиента подняться до
уровня окружающей ткани (отклонение не больше 25 процентов от медианы).

ОТБОР ПЛАСТИН. Мелкий регулярный узор на мятой ткани — худший случай для
...
## Волны, наросты, двойная сетка: гипотеза про denoise (03.09, вечер)

Хозяин перечислил дефекты оборки своими словами: «клетка течёт волнами», «наросты»,
«двойная экспозиция», «белый мусор». Это не одно пятно, это вся нижняя оборка.

Что эти четыре слова означают технически:

| словами хозяина | что это на самом деле |
|---|---|
| клетка течёт волнами | нити не прямые: модель гнёт регулярную решётку по складкам вместо излома |
| наросты | рельефная корка поверх ткани — модель перерисовала само повреждение как объём |
| двойная экспозиция | две сетки под углом друг к другу: старая клетка просвечивает под новой |
| белый мусор | светлые пятна и точки, которых на пластине нет |

Три из четырёх — признаки того, что модель рисует ткань с нуля, а не достраивает
существующую. При `denoise = 1.0` Kontext не обязан опираться на уцелевшие нити:
вход для него — только условие, а не основа. Отсюда и волны, и вторая сетка под углом.

**Правило.** Для восстановления регулярного узора внутри уцелевшей ткани прогонять
ДВА варианта и сравнивать числом:

* `denoise = 1.0` — полная перерисовка, годится там, где узор уничтожен полностью;
* `denoise = 0.65..0.75` — модель держится за уцелевшие нити. FluxGuidance те же 2.5,
  сид тот же, остальное без изменений.

**Чем мерить, кроме периода спектра.** Период ловит масштаб, но не ловит волны и
двойную сетку. Добавить две величины:

1. **Когерентность узора** — доля энергии спектра в 12 главных пиках полосы периодов
   3..21 px. Внутри участка должна быть не ниже, чем снаружи.
2. **Прямизна нитей** — гистограмма направления градиента внутри участка. У прямой
   клетки два узких максимума (два семейства нитей). Волны размазывают распределение,
   двойная экспозиция даёт четыре максимума вместо двух. Это единственный из тестов,
   который ловит «течёт волнами» и «двойную экспозицию» напрямую.

Точечная заплатка 200x200 уже делалась 03.09 (см. `REPORT_FIX_SKIRT.md`) и задачу не
решила: сам отчёт честно отметил, что корка осталась слева от заплатки (x ~ 580..660)
и вдоль края оборки справа (x ~ 850..1140). Брать оборку целиком.
